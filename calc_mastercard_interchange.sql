...
SELECT * FROM hdg.tb_master_categoria_produto; --categoria de produtos mastercard
SELECT * FROM hdg.tb_master_flag_mcc; --flag mcc mastercard
SELECT * FROM hdg.tb_master_grupo_produto; --grupo de produtos mastercard
SELECT * FROM hdg.tb_master_ird; --ird mastercard
SELECT * FROM hdg.tb_master_mcc; --mcc mastercard
SELECT * FROM hdg.tb_master_preco; --preços mastercard
SELECT * FROM hdg.tb_master_produto; --produtos mastercard
SELECT * FROM hdg.tb_master_segmento; --segmentos mastercard
SELECT * FROM hdg.tb_master_sub_categoria_produto; --sub categoria de produtos mastercard
...
v_produto := CASE
	WHEN p_codigo_processamento IN ('002000', '001000') 
THEN COALESCE(p_sub_produto, p_produto)
	ELSE p_produto
END;

SELECT
	round(tmp.taxa_incremental + (p_valor * (tmp.percentual_custo_bandeira + tmp.ajuste_percentual_custo_bandeira + tmp.ajuste_parcela + tmp.ajuste_digital_parcela)) / 100, 2),
